...
`make_printer(cfg)` selects the class based on `cfg.command_set` (`CommandSet.ESCPOS` / `CommandSet.TSPL`, default `escpos`). Both classes expose the same call-site surface — `print_bitmap(img, halign, valign)`, `next_label()` (TSPL is a no-op since `PRINT` already feeds), context manager via `_SerialPrinter`. Constructor signatures diverge: `ESCPrinter` takes `head_alignment` (it has to push the label sideways under a head wider than the paper); `TSPLPrinter` doesn't (the firmware handles paper width via `SIZE` and the head is paper-centred). The `head_alignment` field stays on `LabelConfig` so a config can round-trip its value when temporarily switched to TSPL, but the sidebar only renders the alignment selector when `command_set = escpos`.
